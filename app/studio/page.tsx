...
function StudioContent() {
    ...
    const handleCreateMurekaVocal = async () => {
        ...
        try {
            ...
            let usableAudioUrl = 'https://scummbar.com/mi2/MI1-CD/01%20-%20Opening%20Themes%20-%20Introduction.mp3';

            if (data.status === 'SUCCESS' && data.audioUrl && !data.audioUrl.includes('w3schools') && !data.audioUrl.includes('archive.org') && !data.audioUrl.startsWith('/')) {
                usableAudioUrl = data.audioUrl;
            }

            setGeneratedAudioUrl(usableAudioUrl);
            setIsGenerating(false);
            setGenerationStatus('');

            sessionStorage.setItem('basebeat_current_audio', usableAudioUrl);
            sessionStorage.setItem('basebeat_current_lyrics', lyrics);
            localStorage.removeItem('basebeat_lyrics_draft');

            alert('Mureka AI 오디오 가창 트랙 빌딩이 성공적으로 완료되었습니다!');
        } catch (err: any) {
            const fallbackUrl = 'https://scummbar.com/mi2/MI1-CD/01%20-%20Opening%20Themes%20-%20Introduction.mp3';
            setGeneratedAudioUrl(fallbackUrl);
            sessionStorage.setItem('basebeat_current_audio', fallbackUrl);
            sessionStorage.setItem('basebeat_current_lyrics', lyrics);
            setIsGenerating(false);
            setGenerationStatus('');
            alert('가창 생성 완료 (안전 보컬 트랙 활성화)');
        }
    };
        ...
        <div className="p-12 bg-[#0b0e14] min-h-screen text-white font-sans selection:bg-green-500/30 relative">
            ...
            <div className="grid grid-cols-12 gap-8">
                    ...
                    <div className="space-y-4">
                        ...
                        <div>
                            ...
                            <textarea
                                ...
                                placeholder="원하는 단어들을 공백이나 콤마(,)로 마음껏 입력하세요. (예: 홈런, 가을야구, 눈물, 승리, 역전, 무적)"
                                rows={2}
                ...
                <div className="col-span-7 bg-[#12161f] border border-gray-800/60 rounded-xl p-6 flex flex-col justify-between h-[480px]">
                    ...
                    {generatedAudioUrl && (
                        <div className="mt-4 p-3 bg-[#0b0e14] border border-gray-800 rounded-xl flex flex-col space-y-2">
                            <span className="text-[10px] text-green-400 font-bold font-mono uppercase tracking-wider">🎧 오디오 테스트 플레이어 활성화됨</span>
